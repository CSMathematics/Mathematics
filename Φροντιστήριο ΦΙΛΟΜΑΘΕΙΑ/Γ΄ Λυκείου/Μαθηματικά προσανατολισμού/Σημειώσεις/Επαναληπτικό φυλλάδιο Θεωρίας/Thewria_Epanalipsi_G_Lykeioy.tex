...
row{1} = {bg=red4, fg=white, font=\large\kerkissans},rowhead = 1,
  hline{2} = {.7mm}, % midrule  
}
...
\newcommand{\protashxa}[1]{\refstepcounter{protashxa}{\colorbox{red!80!black}{%
...
$\Delta$ . Πότε λέμε οτι η συνάρτηση $f$ στρέφει τα κοίλα προς τα πάνω ή είναι κυρτή στο $\Delta$;
\end{Orismosbox}
\apanthsh
...
Έστω μια συνάρτηση $f$ συνεχής σε ένα διάστημα $\Delta$ και παραγωγίσιμη στο εσωτερικό του
$\Delta$ . Πότε λέμε οτι η συνάρτηση $f$ στρέφει τα κοίλα προς τα κάτω ή είναι κοίλη στο $\Delta$;
\end{Orismosbox}
\apanthsh
...
\protashxa{Όριο πολυωνυμικής στο άπειρο}
Έστω πολυωνυμική συνάρτηση $f(x)=a_{\nu}x^{\nu}+a_{\nu-1}x^{\nu-1}+\ldots+a_1x+a_0$ με $a_{\nu}\neq 0$. Ισχύει ότι
\[\lim_{x\to\pm\infty}{f(x)}=\lim_{x\to\pm\infty}{\left(a_{\nu}x^{\nu}\right)}\]
\protashxa{Όριο ρητής στο άπειρο}
Έστω ρητή συνάρτηση $f(x)=\dfrac{a_{\nu}x^{\nu}+a_{\nu-1}x^{\nu-1}+\ldots+a_1x+a_0}{\beta_{\mu}x^{\mu}+\beta_{\mu-1}x^{\mu-1}+\ldots+\beta_1x+\beta_0}$ με $a_{\nu}\neq 0$ με $a_{\nu},\beta_{\mu}\neq 0$. Ισχύει ότι
\[\lim_{x\to\pm\infty}{f(x)}=\lim_{x\to\pm\infty}{\frac{a_{\nu}x^{\nu}}{\beta_{\mu}x^{\mu}}}\]
...
\end{itemize}
\protashxa{Σημείο καμπής (Θεώρημα αντίστοιχο του Fermat)}
...
αντίστοιχα
\[\lim\limits_{x\to-\infty}{\frac{f(x)}{x}}=\lambda\in\mathbb{R}\ \ \text{και}\ \ \lim\limits_{x\to -\infty}{[f(x)-\lambda x]}=\beta\in\mathbb{R}\]
\protashxa{Κανόνας De L' Hospital - Μορφή \bmath{$\frac{0}{0}$}}
...
\protashxa{Κανόνας De L' Hospital - Μορφή \bmath{$\frac{\pm\infty}{\pm\infty}$}}
...
\aath{\true}
Αν μια συνάρτηση $f$ δεν είναι $1-1$, τότε δεν είναι και .\\\\
\aaa{\false}
Αν μια συνάρτηση $f$ είναι γνησίως μονότονη σ' ένα διάστημα $\Delta$ του πεδίου ορισμού της, τότε είναι και $1-1$ στο $\Delta$.\\\\
\protash{Θεώρημα Bolzano}\\
\theor{\true}
Θεωρούμε μια συνάρτηση $f$ ορισμένη σε ένα κλειστό διάστημα $[a,\beta]$. Αν
\begin{itemize}
\item η $f$ συνεχής στο κλειστό διάστημα $[a,\beta]$ και 
...
τότε θα υπάρχει τουλάχιστον ένας αριθμός $ x_0\in(a,\beta) $ έτσι ώστε να ισχύει $ f(x_0)=0 $.\\\\
\antistr{\false}
{\kerkissans 1η μορφή:} Έστω συνάρτηση $f$ ορισμένη σε κλειστό διάστημα $[a,\beta]$. Αν η $f$ είναι συνεχής στο κλειστό διάστημα $[a,\beta]$ και ισχύει $ f(x_0)=0 $ για κάποιο $x_0\in[a,\beta]$ τότε:
$f(a)\cdot f(\beta)<0$.\\\\
{\kerkissans 2η μορφή:} Έστω συνάρτηση $f$ ορισμένη σε κλειστό διάστημα $[a,\beta]$. Αν ισχύει $ f(x_0)=0 $ για κάποιο $x_0\in[a,\beta]$ και $f(a)\cdot f(\beta)<0$ τότε η
$f$ είναι συνεχής στο $[a,\beta]$.\\\\
\aath{\true}
\aaa{\false}\\[2cm]
...
\antistr{\false}
Αν μια συνάρτηση $f$ είναι συνεχής σ' ένα σημείο $x_0$, τότε είναι και παραγωγίσιμη στο σημείο αυτό.\\\\
\aath{\true}
Αν μια συνάρτηση $f$ \textbf{δεν} είναι συνεχής σ' ένα σημείο $x_0$, τότε \textbf{δεν} είναι και παραγωγίσιμη στο σημείο αυτό.\\\\
\aaa{\false}
Αν μια συνάρτηση $f$ \textbf{δεν} είναι παραγωγίσιμη σ' ένα σημείο $x_0$, τότε \textbf{δεν} είναι και συνεχής στο σημείο αυτό.\\\\
\newpage
...
\SetCell[r=2,c=1]{bg=red9} $(a,\beta)$ & Γνησίως αύξουσα & $\left(\lim\limits_{x\to a^+}{f(x)},\lim\limits_{x\to \beta^-}{f(x)}\right)$\\
...
\SetCell[r=2,c=1]{bg=red9} $[a,\beta)$ & Γνησίως αύξουσα & $\left[f(a),\lim\limits_{x\to \beta^-}{f(x)}\right)$\\
...
\end{mytblr}
\paragraph{Παραγοντική ολοκλήρωση}
\bmath{$P(x)\cdot e^{ax+\beta}$}\\
\begin{mytblr}{row{2-Z} = {10.7mm}}
Πολυώνυμο & Εκθετική\\
$P(x)$ & $e^{ax+\beta}$\\
$P'(x)$ & $\dfrac{e^{ax+\beta}}{a}$\\
$\vdots$ & $\vdots$\\
$0$ & $\dfrac{e^{ax+\beta}}{a^{\nu}}$
\end{mytblr}

\bmath{$P(x)\cdot \hm{(ax+\beta)}$ ή $\syn{(ax+\beta)}$}\\
\begin{mytblr}{row{2-Z} = {10.7mm}}
Πολυώνυμο & Τριγωνομετρική\\
$P(x)$ & $\hm{(ax+\beta)}$\\
$P'(x)$ & $\dfrac{-\syn{(ax+\beta)}}{a}$\\
$\vdots$ & $\vdots$\\
$0$ & $\pm\dfrac{\hm{(ax+\beta)}}{a^{\nu}}$ ή $\pm\dfrac{\syn{(ax+\beta)}}{a^{\nu}}$
\end{mytblr}\\

\bmath{$e^{ax+\beta}\cdot\hm{(ax+\beta)}$ ή $\syn{(ax+\beta)}$}\\
\begin{mytblr}{row{2-Z} = {10.7mm}}
Τριγωνομετρική & Εκθετική\\
$  $ & $e^{ax+\beta}$\\
$ $ & $\dfrac{e^{ax+\beta}}{a}$\\
$\vdots$ & $\vdots$\\
$0$ & $\dfrac{e^{ax+\beta}}{a^{\nu}}$
