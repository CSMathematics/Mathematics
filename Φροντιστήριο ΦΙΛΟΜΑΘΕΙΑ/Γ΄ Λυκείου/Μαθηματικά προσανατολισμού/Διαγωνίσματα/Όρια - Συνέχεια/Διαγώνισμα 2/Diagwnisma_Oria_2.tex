...
\begin{erwthma}
\item $ \lim_{x\to 0}{f(x)} $\monades{5}
\item $ \lim_{x\to 0}{\dfrac{f(x)-f(0)}{x}} $\monades{8}
\item $ \lim_{x\to 1}{\dfrac{f(x-1)\left( 1-\syn^2{(x-1)}\right) }{x^2-x}} $\monades{7}
\item $ \lim_{x\to 0}{\dfrac{f^2(x)+f(x)-2}{f^2(x)-f(x)}} $\monades{7}
\end{erwthma}\mbox{}\\
\item \mbox{}\\
...
\begin{erwthma}
\item Να αποδείξετε ότι $ a=-2 $ και $ \beta=-3 $.\monades{8}
\item Να βρείτε τα όρια $ \lim_{x\to +\infty}{f(x)} $ και $ \lim_{x\to -\infty}{f(x)} $.\monades{9}
\item Να υπολογίσετε το όριο $ \lim_{x\to 1^+}{\dfrac{f(x)-f(1)}{x-1}} $.\monades{8}
\end{erwthma}\mbox{}\\
\item \mbox{}\\
Δίνεται η συνάρτηση $ f:\mathbb{R}\to\mathbb{R} $, για την οποία ισχύει:
