...
\vspace{10mm}\mbox{}\\
\centering
\kerkissans{{\huge Μαθηματικά προσανατολισμού - Γ' Λυκείου}\\\vspace{2mm} {\LARGE Κανόνας \eng{De L' Hospital}}}\\\vspace{4mm}{\kerkissans{\today}}\\\vspace{3mm}}]
\paragraph{Μορφή $\dfrac{0}{0}$ και $\dfrac{\pm\infty}{\pm\infty}$}
\askhsh Να υπολογίσετε τα παρακάτω όρια.
\begin{multicols}{2}
\begin{alist}
\item $\displaystyle{\lim_{x\to 0}\frac{e^x-1}{x}}$
\item $\displaystyle{\lim_{x\to 0}\frac{\ln{(x+1)}}{x}}$
\item $\lim\limits_{x\to 0}{\dfrac{\ln{(x+1)}}{e^x-1}}$
\item $\lim\limits_{x\to 0}{\dfrac{x^3}{\hm{x}-x}}$
\item $\lim\limits_{x\to 0}{\dfrac{e^x-x-1}{x^2}}$
\item $\lim\limits_{x\to 0}{\dfrac{xe^x}{}}$
\end{alist}
\end{multicols}
\paragraph{Μορφή $0\cdot(\pm\infty)$}
\askhsh
\paragraph{Μορφή $\infty-\infty$}
\askhsh
\paragraph{Μορφή $0^0,1^{\pm\infty}$ και $(\pm\infty)^0$}
\askhsh 
\end{document}
