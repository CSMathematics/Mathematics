...
\begin{document}
Περιοδική συνάρτηση\\
Υπάρχει $Τ>0$ ώστε για κάθε $x\in A$
\begin{itemize}[noitemsep]
\item $x+T\in A$ και $x-T\in A$
\item $f(x+T)=(x-T)=f(x)$
\end{itemize}
\bmath{$f(x)=\rho\cdot\hm(\omega x)$ και $f(x)=\rho\cdot\syn{(\omega x)}$}
\begin{itemize}[noitemsep]
\item Περίοδος: $T=\dfrac{2\pi}{\omega}$
\item Μέγιστο: $|\rho|$
\item Ελάχιστο: $-|\rho|$
\end{itemize}
\bmath{$f(x)=\rho\cdot\ef(\omega x)$ και $f(x)=\rho\cdot\syf{(\omega x)}$}
\begin{itemize}[noitemsep]
\item Περίοδος: $T=\dfrac{\pi}{\omega}$
\item Δεν έχουν ακρότατα.
\item Ασύμπτωτες εφαπτομένης: $x=\dfrac{\kappa\pi+\frac{\pi}{2}}{\omega},\ \kappa\in\mathbb{Z}$
\item Ασύμπτωτες συνεφαπτομένης: $x=\dfrac{\kappa\pi}{\omega},\ \kappa\in\mathbb{Z}$
\end{itemize}
\end{document}
