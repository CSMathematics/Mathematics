...
\usepackage{amsmath}
\usepackage[amsbb,mtpfrak,zswash,mtpcal]{mtpro2}
\usepackage{makeidx,eurosym,array}
\usepackage{longtable,xcolor,varwidth}
\usepackage{float}
%\usepackage{subfig}
\def\xrwma{cyan}
\def\xrwmath{cyan!70!black}
\usepackage{etoolbox}
\makeatletter
\newif\ifLT@nocaption
...
\begin{tikzpicture}
\path[left color=black,right color=black,middle color=black!30!white] (-9cm,-.6cm) rectangle (8.5cm,.6cm);
...
%------ ΘΕΩΡΙΑ ΤΙΤΛΟΣ ---------
\newcommand{\Thewria}{\begin{center}
\begin{tikzpicture}
\path[left color=black,right color=black,middle color=black!30!white] (-9cm,-.6cm) rectangle (8.5cm,.6cm);
\node at (-.25cm,0) {\Large \textcolor{white}{\textbf{ΘΕΩΡΙΑ}}};  
\end{tikzpicture}
\end{center}}
%--------------------------------------
...
colback=white,title={\large Μέθοδος \thechapter.\theMethodos} : {\textcolor{black}{#1}}]}
{\end{tcolorbox}}
%------------------------------------------
%-------- ΠΑΡΑΤΗΡΗΣΕΙΣ -----------------
...
enhanced,lifted shadow={1mm}{-2mm}{3mm}{0.3mm}%
{black!50!white},
...
\setlist[bhma]{label=\bf\textit{\arabic*\textsuperscript{o}\;Βήμα :},itemsep=0mm,leftmargin=0cm,itemindent=1.7cm,ref=\bf{\arabic*\textsuperscript{o}\;Βήμα}}
...
\part{Άλγεβρα}
\pagestyle{fancy}
%\input{Pr_Arithmoi.tex}
\input{Eksiswseis.tex}
\part{Γεωμετρία}
\backmatter
\pagestyle{empty}
\listoffigures
