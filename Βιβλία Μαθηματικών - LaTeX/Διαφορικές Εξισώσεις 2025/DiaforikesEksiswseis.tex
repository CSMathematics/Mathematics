...
% ΠΛΗΡΟΦΟΡΙΑ
\newenvironment{Plhroforia}[1]
{\begin{tcolorbox}[title=\textbf{\faBook\ \ \ \titlefont{Πληροφορία}},
breakable,
enhanced standard,
lifted shadow={1mm}{-2mm}{3mm}{0.3mm}{black!50!white},
...
\end{equation}
όπου $g(y)$ συνεχής στο διάστημα $\varDelta$. Έπειτα παραγωγίζουμε την παραπάνω σχέση ως προς $y$ και προσδιορίζουμε τη συνάρτηση $g(y)$:
\begin{gather*}
\diffp{f}{y}=\diffp{}{y}\left[\int{M(x,y)\d x}+g(y)\right]\Rightarrow N(x,y)=\int{\diffp{M}{y}\d x}+g'(y)\Leftrightarrow\\
N(x,y)=\int{\diffp{N}{x}\d x}+g'(y)\Leftrightarrow g(y)=\int{\left(N(x,y)-\int{N_x\d x}\right)\d y}
\end{gather*}
Αντικαθιστούμε την $g(y)$ στη σχέση \eqref{eq:exact:5} και οδηγούμαστε έτσι στη λύση της εξίσωσης. Η διαδικασία ακολουθεί τα ίδια βήματα κι στην περίπτωση όπου επιλέξουμε να ξεκινήσουμε από τη δεύτερη σχέση της \eqref{eq:exact:5}. την οποία θα ολοκληρώσουμε ως προς $y$. Θα έχουμε 
...
\begin{gather*}
\diffp{f}{x}=\diffp{}{x}\left[\int{N(x,y)\d y}+h(x)\right]\Rightarrow M(x,y)=\int{\diffp{N}{x}\d y}+h'(x)\Rightarrow\\
M(x,y)=\int{\diffp{M}{y}\d y}+h'(x)\Rightarrow h(x)=\int{\left(M(x,y)-\int{M_y}\d y\right)\d x}
\end{gather*}
οποία και αντικαθιστούμε στην \eqref{eq:exact:6} και προκύπτει η πεπλεγμένη μορφή της λύσης.\\\\
