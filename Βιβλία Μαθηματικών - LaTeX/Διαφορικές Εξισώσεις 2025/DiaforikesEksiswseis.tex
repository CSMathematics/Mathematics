...
%------------------- Γεωμετρία - Στύλ ----------------------------------
\usepackage[inner=1.50cm, top=3.00cm, bottom=2.00cm]{geometry}
\geometry{textwidth=12cm,marginparsep=9mm,marginparwidth=5cm}
...
hlines={white},
vlines={white},
row{1} = {bg=red4, fg=white, font=\bfseries\fontfamily{maksf}},
rowhead = 1,
hline{2} = {.7mm}, % midrule  
}
%-------------------------------------------------------------------
...
Λύση ή ολοκλήρωμα μιας συνήθους διαφορικής εξίσωσης της μορφής \eqref{eq:linode1:1} ονομάζεται κάθε συνάρτηση $y(x)$ που επαληθεύει την εξίσωση για κάθε $x\in[a,\beta]$.
\end{Orismos}

\section{Προβλήματα αρχικών κι συνοριακών τιμών}
...
\end{equation}
\begin{Paradeigma}{Γραμμική 1ης τάξης}
Να βρεθεί η γενική λύση της γραμμικής διαφορικής εξίσωσης
\[xy'+ y = 0\]
και να λυθεί το πρόβλημα αρχικών τιμών $xy'+y=0,\ y(1)=-2$.
\end{Paradeigma} 
...
\addplot[grafikh parastash,domain=-7:7,RoyalBlue]{1/x};
\addplot[grafikh parastash,domain=-7:7,thirdcolor]{2/x};
\addplot[grafikh parastash,domain=-7:7,secondarycolor]{3/x};
\addplot[grafikh parastash,domain=-7:7,RoyalBlue]{-1/x};
\addplot[grafikh parastash,domain=-7:7,thirdcolor]{-2/x};
\addplot[grafikh parastash,domain=-7:7,secondarycolor]{-3/x};
\end{scope}
\node at (axis cs:-.5,-0.5) {\footnotesize$O$};
\legend{${c=1,-1}$,${c=2,-2}$,${c=3,-3}$}
\end{axis}
...
\begin{scope}
\clip (axis cs:-7,-7) rectangle (axis cs:7,7);
\addplot[grafikh parastash,domain=-7:7,RoyalBlue]{-2/x};
\end{scope}
\node at (axis cs:3,3) {$y(x)=-\frac{2}{x}$};
\node at (axis cs:-.5,-0.5) {\footnotesize$O$};
\end{axis}
\end{tikzpicture}\captionof{figure}{Ειδική λύση του ΠΑΤ}\label{plot:linode1:2}
\end{marginfigure}
...
\[ y(x)=ce^{\dintt{\frac{1}{x}\d x}}=ce^{\ln{\frac{1}{x}}}=\frac{c}{x} \]
όπου $c$ μια αυθαίρετη σταθερά.
Στο \ref{plot:linode1:1} δείχνουμε τα γραφήματα κάποιων λύσεων της  \eqref{eq:linode1:1} που αντιστοιχούν σε διάφορες τιμές της σταθεράς $c$. Σύμφωνα τώρα με την αρχική συνθήκη έχουμε
\[ y(1)=-2\Rightarrow c=-2 \]
έτσι η λύση του Π.Α.Τ. θα είναι η $y(x)=-\frac{2}{x}$, της οποίας η γραφική παράσταση φαίνεται στο \ref{plot:linode1:2}.
