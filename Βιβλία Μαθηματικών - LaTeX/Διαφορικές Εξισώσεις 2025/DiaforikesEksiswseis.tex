...
% Ορίζουμε τις "οικογένειες" γραμματοσειρών
\newfontfamily\notoserif{Noto Serif}
\newfontfamily\urwgothic{URW Gothic}

% Ορίζουμε εντολές που τις χρησιμοποιούν με το σωστό μέγεθος
...
\usepackage{mathimatika,venndiagram,diffcoeff,mathtools}
\allowdisplaybreaks
...
\full{\Alyta}
\fulltwoc{
\Askhsh Επαληθεύστε ότι οι ακόλουθες εξισώσεις είναι πλήρεις και βρείτε τη γενική λύση τους.
\begin{alist}
\item $x\d x+y\d y=0$
\item $y\d x+x\d y=0$
\item $ye^x\d x+(e^x+y)\d y=0$
\item $y\cos{x}\d x+\sin{x}\d y=0$
\item $\tan{y}\d x+\dfrac{x}{\cos^2{y}}\d y=0$
\item $\left(3x^3+y^3\right)\d x+3xy^2\d y=0$
\end{alist}
\Askhsh Ελέγξτε ποιες από τις παρακάτω εξισώσεις είναι πλήρεις. Για όσες δεν είναι πλήρεις, βρείτε έναν ολοκληρωτικό παράγοντα $\mu$ της μορφής $\mu(x)$ ή $\mu(y)$ καθώς και τη λύση τους.
\begin{alist}
\item $2y\d x+x\d y=0$
\item $2xy\ln{y}\d x+x^2\d y=0$
\item $\left(e^x\ln{y}\right)\d x+\dfrac{e^x}{y}\d y=0$
\end{alist}
\Askhsh Δίνεται η διαφορική εξίσωση
\[ 3xy\d x+2x^2\d y=0 \]
\begin{alist}
\item Δείξτε ότι η εξίσωση δεν είναι πλήρης.
\item Βρείτε έναν ολοκληρωτικό παράγοντα της  εξίσωσης, της μορφής $\mu(x,y)=x^{\nu}y^{\nu}$ με $\nu\in\mathbb{N}$.
\item Βρείτε τη γενική λύση της διαφορικής εξίσωσης.
\end{alist}
\Askhsh \lipsum[1]
}
\full{\alyta}
\section{Ομογενείς διαφορικές εξισώσεις}
\begin{Orismos}{Ομογενής εξίσωση}
Η διαφορική εξίσωση
\[ y'=f(x,y) \]
λέγεται ομογενής αν η συνάρτηση $f$ εξαρτάται από το λόγο $\frac{y}{x}$... Να συμπληρώσω το $f(tx,ty)=t^nf(x,y)$...
\end{Orismos}
\paragraph{Ο μετασχηματισμός $y=zx$}
Η χρήση του μετασχηματισμού $y=zx$ σε μια ομογενή εξίσωση αποτελεί κλασσικό τρόπο επίλυσης και μετατρέπει την αρχική διαφορική εξίσωση σε μία χωριζομένων μεταβλητών...?\\\\

\begin{Paradeigma}{Ομογενής εξίσωση}\label{par:omogeneis:1}
Βρείτε τη γενική λύση της εξίσωσης
\[ y'=\frac{y^2+2xy}{x^2} \]
\end{Paradeigma}
\begin{marginfigure}[0mm]
\begin{Parathrhsh}{5.2cm}
Η αρχική διαφορική εξίσωση αποτελεί εξίσωση \eng{Bernoulli} γιατί γράφεται
\begin{gather*}
y'=\frac{y^2+2xy}{x^2}\Leftrightarrow\\
y'-\frac{2}{x}y=\frac{1}{x^2}y^2
\end{gather*}
και μπορεί να αντιμετωπιστεί με τη μέθοδο που θα συναντήσουμε στην παράγραφο \ref{sec:Bernoulli}.
\end{Parathrhsh}
\end{marginfigure}
Μπορούμε να δείξουμε ότι η αρχική εξίσωση είναι ομογενής με το να σχηματίσουμε το λόγο $\frac{y}{x}$ στο δεύτερο μέλος της. Έχουμε λοιπόν
\[ y'=\frac{y^2+2xy}{x^2}=\frac{y^2}{x^2}+\frac{2xy}{x^2}=\left(\frac{y}{x}\right)^2+2\frac{y}{x} \]
Στη συνέχεια με το μετασχηματισμό $y=zx$ έχουμε αρχικά $y'=z'x+z$. Αντικαθιστώντας θα καταλήξουμε σε μία εξίσωση χωριζομένων μεταβλητών.
\begin{gather*}
z'x+z=z^2+2z\Leftrightarrow z'x=z+z^2\Leftrightarrow\\ \diffp{z}{x}x=z+z^2\Leftrightarrow \frac{\d z}{z^2+z}=\frac{\d x}{x}
\end{gather*}
...
\full{\Alyta}
\fulltwoc{
\Askhsh Δείξτε ότι η ομογενής διαφορική εξίσωση του παραδείγματος {\ref{par:omogeneis:1}} αποτελεί και εξίσωση \eng{Bernoulli} και στη συνέχεια βρείτε τη γενική λύση της.
}
\full{\alyta}
\lipsum[1-2]
...
\setchapterimage{./images/11.jpeg}
