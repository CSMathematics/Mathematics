...
\item Αν $\varepsilon: y=-\frac{x}{2}+\frac{3}{2}$ είναι η εξίσωση της εφαπτομένης στο $A$, να δείξετε ότι ισχύει $2\sqrt{x}+x-3\geq\ln{x}$ για κάθε $x>0$.
...
\item να βρεθεί το εμβαδόν του χωρίου που περικλείεται από την $C_f$, τον άξονα $x'x$ και την ευθεία $x=-2$.
\end{erwthma}
\end{askhsh}

\begin{askhsh}{Β}
Δίνονται οι συναρτήσεις $g:(0,+\infty)\to\mathbb{R}$ με τύπο $g(x)=\ln{\left(x-\frac{1}{x}\right)}$ και $h:\mathbb{R}\to\mathbb{R}$ με $h(x)=e^x$.
\begin{erwthma}
...
\[ \lim_{x\to+\infty}{\frac{f^{-1}(x)}{x}} \]
\end{erwthma}
\end{askhsh}
\part{Επαναληπτικά Θέματα Γ}
\begin{askhsh}{Γ}
Δίνεται συνάρτηση $f:(0,+\infty)\to\mathbb{R}$ για την οποία ισχύει
\[ x^2f'(x)+xf(x)=1\ , \ x>0 \]
Να δείξετε ότι:
\begin{erwthma}
\item ο τύπος της $f$ είναι $f(x)=\frac{\ln{x}}{x}$ για κάθε $ x>0$.
...
\item Να υπολογίσετε το όριο \[ \lim_{x\to-\infty}{\frac{f(x)}{xf'(x)}} \]
\item Μελετήστε την $f$ ως προς την κυρτότητα και τα σημεία καμπής.
