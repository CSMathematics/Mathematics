\begin{thema}{Β}
Στο παρακάτω σχήμα δίνεται η γραφική παράσταση της \textbf{παραγώγου} $f'$ μιας συνεχούς συνάρτησης $f$ ορισμένης στο $[-2, 5]$. Η $C_{f'}$ αποτελείται από δύο ευθύγραμμα τμήματα: το πρώτο ενώνει τα σημεία $(-2, -2)$ και $(1, 4)$, και το δεύτερο ενώνει τα σημεία $(1, 4)$ και $(5, 0)$.

\begin{center}
\begin{tikzpicture}[scale=0.75]
  % grid
  \draw[help lines, gray!30] (-3,-3) grid (6,5);
  % axes
  \draw[-latex] (-3.3,0) -- (6.3,0) node[right] {$x$};
  \draw[-latex] (0,-3.3) -- (0,5.3) node[above] {$y$};
  % ticks
  \foreach \x in {-2,-1,1,2,3,4,5}
    \draw (\x,0.1) -- (\x,-0.1) node[below,font=\footnotesize] {$\x$};
  \foreach \y in {-2,-1,1,2,3,4}
    \draw (0.1,\y) -- (-0.1,\y) node[left,font=\footnotesize] {$\y$};
  % f' curve (two line segments)
  \draw[very thick, red!70!black] (-2,-2) -- (1,4) -- (5,0);
  % shade positive area
  \fill[blue!10, opacity=0.5] (-2,-2) -- (-1,0) -- (-2,0) -- cycle;
  \fill[orange!10, opacity=0.5] (-1,0) -- (1,4) -- (5,0) -- (-1,0) -- cycle;
  % key points
  \filldraw[red!70!black] (-2,-2) circle (2pt);
  \filldraw[red!70!black] (1,4) circle (2pt) node[above right] {$(1,4)$};
  \filldraw[red!70!black] (5,0) circle (2pt);
  \filldraw[red!70!black] (-1,0) circle (2pt) node[above left,font=\footnotesize] {$(-1,0)$};
  % label
  \node[red!70!black] at (3.5,3) {$C_{f'}$};
\end{tikzpicture}
\end{center}

\begin{erwthma}
  \item Να μελετήσετε τη μονοτονία της $f$ και να βρείτε τα ακρότατά της.
  \item Να υπολογίσετε τη διαφορά $f(5) - f(-1)$ χρησιμοποιώντας το εμβαδόν του χρωματισμένου χωρίου.
  \item Αν $f(1) = 3$, να βρείτε την εξίσωση της εφαπτομένης της $C_f$ στο σημείο με τετμημένη $x_0 = 1$.
  \item Να μελετήσετε την κυρτότητα της $f$ στο $[-2, 5]$.
\end{erwthma}
\end{thema}
